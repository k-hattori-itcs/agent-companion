...
- Claude Desk の起動先は Windows のスタートアプリ一覧から実行時に検出するため、端末固有の識別子に依存しません。
- Claude Code OAuth の利用量取得で、期限切れ・期限間近の資格情報を安全に更新し、401時は更新後に一度だけ再試行します。
- Claude API が429を返した場合は `Retry-After` を保存し、再起動後も指定時刻まで再試行しません。
- 既存の Claude 監視設定は、初回読み込み時に従来どおり VSCode 起動へ移行します。

## 画面イメージ

spritesheet から切り出したキャラクタープレビューを使った公開用の画面イメージです。利用率と吹き出しは固定サンプル値であり、実行履歴・リセット日時・個人環境の情報は含みません。
...
- Windows 10 / 11
- GitHub Releases の配布ZIPを使う場合、.NET Runtime の別途インストールは不要
- ソースからビルドする場合は .NET 8 SDK

## 導入・配布

利用者向けの導入、ハッシュ確認、GitHub Attestation検証、更新、アンインストールは [INSTALL.md](./INSTALL.md) を参照してください。Release ZIPはGitHub Actionsでビルドし、GitHub Attestationで来歴を検証できます。WindowsのAuthenticode署名は付与していません。 公開管理者は [RELEASE.md](./RELEASE.md) の手順に従います。
## 使い方
...
1. 設定で有効化した場合のみ、Claude Code OAuthによる利用状況APIの実値
...
8x11のv2スプライトを追加する場合は、spriteVersionNumber: 2 を指定し、1536x2288px・1セル192x208px・8列11行の画像を使ってください。spritesheetLayout を記載する場合も、この値（columns: 8、rows: 11、cellWidth: 192、cellHeight: 208、lookDirectionCount: 16）と一致する必要があります。rows 0〜8がアプリで再生する標準9動作、rows 9〜10は将来の視線方向用データとしてパッケージ内に保持されます。
...
共有PCでは、OSのファイル権限を使って `%LOCALAPPDATA%\AgentCompanion`、`.codex`、`.claude` を他ユーザーから保護してください。ログや設定をIssueへ添付する前に、ローカルパスや会話内容が含まれていないか確認してください。
## ライセンス
