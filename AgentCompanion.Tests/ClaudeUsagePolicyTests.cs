...
public sealed class ClaudeUsagePolicyTests
{
    ...
    [Fact]
    public void FreshApiUsageProvidesResetTimeForRequestedWindow()
    {
        var now = Now();
        var cache = Cache("C:\\Users\\test\\.claude", now, fiveHour: 73, sevenDay: 57);

        var result = ClaudeUsagePolicy.GetFreshResetAt(
            cache,
            cache.ClaudeHome,
            ClaudeUsageWindow.SevenDay,
            now);
    ...
    [Fact]
    public void StaleApiUsageKeepsFutureResetTimeVisible()
    {
        var now = Now();
        var cache = Cache("C:\\Users\\test\\.claude", now.AddMinutes(-6), fiveHour: 73, sevenDay: 57);
        ...
        Assert.Equal(now.AddMinutes(54), result);
    }

    [Fact]
    public void ExpiredResetTimeIsRetainedUntilTheNextRefresh()
    {
        var now = Now();
        var response = new ClaudeUsageApiResponse(73, now.AddMinutes(-2), 57, now.AddDays(1));
        var cache = new ClaudeUsageApiCache("C:\\Users\\test\\.claude", response, now.AddHours(-1));
        ...
        Assert.Equal(now.AddMinutes(-2), result);
    }
