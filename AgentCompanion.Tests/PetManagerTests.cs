...
public sealed class PetManagerTests : IDisposable
{
    ...
    [Fact]
    public void ImportPet_RejectsV2PackageWithNonStandardAtlasDimensions()
    {
        var zip = CreatePackage("safe-pet", "spritesheet.png", spriteVersionNumber: 2);
    ...
    [Fact]
    public void ImportPet_PreservesDeclaredSpriteMetadata()
    {
        var zip = CreatePackage("safe-pet", "spritesheet.png", spriteVersionNumber: 1, includeLayout: true);

        var result = new PetManager(PetsDirectory).ImportPet(zip);

        Assert.Null(result);
        using var document = JsonDocument.Parse(File.ReadAllText(Path.Combine(PetsDirectory, "safe-pet", "pet.json")));
        Assert.Equal(1, document.RootElement.GetProperty("spriteVersionNumber").GetInt32());
        var layout = document.RootElement.GetProperty("spritesheetLayout");
        Assert.Equal(8, layout.GetProperty("columns").GetInt32());
        Assert.Equal(11, layout.GetProperty("rows").GetInt32());
        Assert.Equal(16, layout.GetProperty("lookDirectionCount").GetInt32());
        Assert.Equal(0, layout.GetProperty("neutralLookFrame").GetProperty("rowIndex").GetInt32());
    }
    [Fact]
    ...
    [Fact]
    public void ImportPet_ReplacesExistingCharacterFilesInPlace()
    {
        CreateInstalledPet("safe-pet");
        var petDir = Path.Combine(PetsDirectory, "safe-pet");
        File.WriteAllText(Path.Combine(petDir, "preview-idle.png"), "stale preview");
        var manager = new PetManager(PetsDirectory);
        manager.Setup();
        var zip = CreatePackage("safe-pet", "spritesheet.png");
    ...
    public void Setup_AppliesLunaSittingScaleWhenManifestDoesNotSpecifyOne()
    {
        ...
        var luna = Assert.Single(manager.Pets, pet => pet.Id == "luna");
        Assert.Equal(0.9, luna.AnimationScales["sit"]);
    }

    [Fact]
    public void Setup_AppliesNatsukiJumpAndSittingScalesWhenManifestDoesNotSpecifyThem()
    {
        CreateInstalledPet("natsuki");
        var manager = new PetManager(PetsDirectory);

        manager.Setup();

        var natsuki = Assert.Single(manager.Pets, pet => pet.Id == "natsuki");
        Assert.Equal(0.9, natsuki.AnimationScales["jump"]);
        Assert.Equal(0.84, natsuki.AnimationScales["sit"]);
    }
        ...
        bool includeLayout = false)
    {
        ...
        var manifest = new
        {
            ...
            spriteVersionNumber,
            spritesheetLayout = includeLayout
                ? new
                {
                    columns = 8,
                    rows = 11,
                    ...
                    neutralLookFrame = new { rowIndex = 0, columnIndex = 0 }
                }
                : null,
            frameWidth = 128,
            frameHeight = 128,
