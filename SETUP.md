...
GitHub Releases の配布ZIPを使う場合、.NET Runtime の別途インストールは不要です。
...
初回起動時、設定ファイルとキャラクターデータが `%LOCALAPPDATA%\AgentCompanion\instances\<instance-id>` に作成されます。
...
同じ実行フォルダから起動した AgentCompanion は同じプロファイルを使います。別設定で複数運用したい場合は、フォルダを2つに分けてください。
...
それぞれのフォルダで一度起動し、設定画面から接続先プロファイルとキャラクターの見た目を個別に設定します。設定やキャラクターは `%LOCALAPPDATA%\AgentCompanion\instances\<instance-id>` に保存され、`<instance-id>` は実行フォルダのフルパスごとに変わります。
...
8x11のv2スプライトを追加する場合は、spriteVersionNumber: 2 とし、1536x2288px（1セル192x208px、8列11行）の画像を指定してください。spritesheetLayout を書く場合も columns: 8、rows: 11、cellWidth: 192、cellHeight: 208、lookDirectionCount: 16 と一致させます。rows 0〜8がアプリで再生する標準9動作です。インポート時にこの構成と画像サイズを検証するため、条件を満たさないv2 ZIPは既存キャラクターを置き換えません。

削除機能は設定画面には出していません。不要なキャラクターを消したい場合は、設定画面の `フォルダを開く` で保存先を開き、アプリを終了してから `pets` 配下の対象フォルダを手動で削除してください。
