...
public partial class PetSprite : UserControl
{
    ...
    private IReadOnlyDictionary<string, double> _animationScales = new Dictionary<string, double>();
    ...
    public void SetAnimationScales(IReadOnlyDictionary<string, double>? animationScales)
    {
        _animationScales = animationScales ?? new Dictionary<string, double>();
        ApplyAnimationScale();
    }
    ...
    {
        var scale = _animationScales.TryGetValue(_currentAnim, out var configuredScale)
            ? Math.Clamp(configuredScale, 0.6, 1.0)
            : 1.0;
        var transform = new TransformGroup();
        transform.Children.Add(new ScaleTransform(scale, scale));
        transform.Children.Add(new TranslateTransform(0, GetAnimationVerticalOffset()));
        SpriteImage.RenderTransformOrigin = new Point(0.5, 1.0);
        SpriteImage.RenderTransform = transform;
    }

    private double GetAnimationVerticalOffset()
    {
        if (_currentAnim != "jump")
            return 0;

        return _currentFrame switch
        {
            1 or 3 => -12,
            2 => -20,
            _ => 0
