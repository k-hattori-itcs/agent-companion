...
public partial class MainWindow : Window
{
    ...
    private const double RingBadgeReservedHeight = 30;
    ...
    private bool _claudeUsageCacheRestored;
    ...
    internal void LoadPetSprite()
    {
        ...
        var activePet = _app.PetManager.Pets.FirstOrDefault(pet => pet.Id == _app.PetManager.ActivePetId);
        PetSprite.SetAnimationScales(activePet?.AnimationScales);
    ...
    internal void SnapWindowToPet()
    {
        ...
        RingBadge.Margin = new Thickness(0, 2, 0, 0);

        Width = Math.Max(stageWidth, 240);
        Height = stageHeight + BubbleReservedHeight + RingBadgeReservedHeight;
        UpdateTokenRing();
    }
    ...
    private void RefreshAgentStatus()
    {
        ...
        CodexStatusSnapshot currentStatus;
        if (IsClaudeProvider())
        {
            RestoreClaudeUsageCacheIfNeeded();
            currentStatus = _claudeStatus.Poll(
                ...
                _app.Config.ClaudeUsageApiEnabled);
            PersistClaudeUsageCache();
        }
        else
        {
            currentStatus = _codexStatus.Poll();
        }
    ...
    private void RestoreClaudeUsageCacheIfNeeded()
    {
        if (_claudeUsageCacheRestored)
            return;

        _claudeUsageCacheRestored = true;
        _claudeStatus.RestoreCachedApiUsage(
            _app.Config.ClaudeUsageCacheHome,
            _app.Config.ClaudeFiveHourUsagePercent,
    ...
    private void UpdateTokenRing()
    {
        ...
        if (IsClaudeProvider() && secondaryPercent.HasValue)
        {
            ...
            var usageLabel = TokenRingTextFormatter.FormatClaudeUsage(primaryLabel, secondaryLabel);
            var resetLine = TokenRingTextFormatter.FormatResetLine(
                _lastCodexStatus?.TokenUsageResetsAt,
                _lastCodexStatus?.SecondaryTokenUsageResetsAt,
                includeSecondary: true);
            RingBadgeText.Text = usageLabel + resetLine;
        }
        else
        {
            var usageLabel = statusPercent.HasValue
