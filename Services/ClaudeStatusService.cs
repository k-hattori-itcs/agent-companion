...
public sealed class ClaudeStatusService
{
    ...
    internal void RestoreCachedApiUsage(
        string claudeHome,
        ...
        bool useUsageApi = false)
    {
        ...
        if (string.IsNullOrWhiteSpace(_latestTranscriptPath) || !File.Exists(_latestTranscriptPath))
            return WithUsage(_lastSnapshot, claudeHome, shortWindowTokenLimit, weeklyTokenLimit, shortWindowHours, useUsageApi);
    ...
    private CodexStatusSnapshot WithUsage(
        CodexStatusSnapshot status,
        string claudeHome,
        long shortWindowTokenLimit,
        long weeklyTokenLimit,
        double shortWindowHours,
        bool useUsageApi)
    {
        ...
        return status with
        {
            ...
            TokenUsageResetsAt = ClaudeUsagePolicy.GetFreshResetAt(apiUsage, claudeHome, ClaudeUsageWindow.FiveHour, now) ?? _lastUsage.ShortWindowResetsAt,
            SecondaryTokenUsageResetsAt = ClaudeUsagePolicy.GetFreshResetAt(apiUsage, claudeHome, ClaudeUsageWindow.SevenDay, now) ?? _lastUsage.WeeklyResetsAt
        };
    }
    ...
    private static ClaudeUsageSnapshot BuildUsageSnapshot(string claudeHome, double shortWindowHours)
    {
        var projectsDir = Path.Combine(claudeHome, "projects");
        if (!Directory.Exists(projectsDir))
            return new ClaudeUsageSnapshot(0, 0, null, null, null, null, null);
        ...
        return new ClaudeUsageSnapshot(
            ...
            weeklyTokens,
            latestTurnTokens,
            rateLimits?.ShortWindow?.Percent,
            rateLimits?.Weekly?.Percent,
            rateLimits?.ShortWindow?.ResetsAt,
    ...
    private static ClaudeStatuslineRateLimits? ReadStatuslineRateLimits(string claudeHome)
    {
        ...
        try
        {
            ...
            var now = DateTime.Now;
            var fiveHour = ExtractStatuslineLimit(root, "five_hour", now, fileUpdatedAt);
            var sevenDay = ExtractStatuslineLimit(root, "seven_day", now, fileUpdatedAt);
    ...
    private static ClaudeStatuslineLimit? ExtractStatuslineLimit(JsonElement rateLimits, string key, DateTime now, DateTime fileUpdatedAt)
    {
        ...
        return new ClaudeStatuslineLimit(Math.Clamp(percent.Value, 0, 100), resetsAt == default ? null : new DateTimeOffset(resetsAt));
    }
    ...
    private sealed record ClaudeUsageSnapshot(
        long ShortWindowTokens,
        long WeeklyTokens,
        long? LatestTurnTokens,
        double? ShortWindowPercent,
        double? WeeklyPercent,
        DateTimeOffset? ShortWindowResetsAt,
        DateTimeOffset? WeeklyResetsAt);
    private sealed record ClaudeStatuslineRateLimits(ClaudeStatuslineLimit? ShortWindow, ClaudeStatuslineLimit? Weekly);
    private sealed record ClaudeStatuslineLimit(double Percent, DateTimeOffset? ResetsAt);
