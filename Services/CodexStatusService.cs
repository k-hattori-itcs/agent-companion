...
public sealed record CodexStatusSnapshot(
    ...
    DateTimeOffset? TokenUsageResetsAt = null,
    DateTimeOffset? SecondaryTokenUsageResetsAt = null);
...
public sealed class CodexStatusService
{
    ...
    private CodexStatusSnapshot ParseRollout(string rolloutPath, DateTime fileUpdatedAt)
    {
            ...
            try
            {
                ...
                if (type == "event_msg")
                    ApplyEventMessage(
                        ...
                        ref summary,
                        ref isRunning,
                        ref tokenUsagePercent,
                        ref tokenUsageResetsAt,
                        ref lastTurnTokens);
        ...
        return new CodexStatusSnapshot(
            ...
            updatedAt,
            rolloutPath,
            TokenUsageResetsAt: tokenUsageResetsAt);
    }
    ...
    private static void ApplyEventMessage(
        JsonElement payload,
        ref string summary,
        ref bool isRunning,
        ref double? tokenUsagePercent,
        ref DateTimeOffset? tokenUsageResetsAt,
        ref long? lastTurnTokens)
    {
        ...
        switch (payloadTypeProp.GetString())
        {
            ...
            case "token_count":
                ...
                tokenUsageResetsAt = ExtractPrimaryLimitReset(payload) ?? tokenUsageResetsAt;
    ...
    private static DateTimeOffset? ExtractPrimaryLimitReset(JsonElement payload)
    {
        if (!payload.TryGetProperty("rate_limits", out var rateLimits)) return null;
        if (!rateLimits.TryGetProperty("primary", out var primary)) return null;
        if (!primary.TryGetProperty("resets_at", out var resetsAt)) return null;
        if (resetsAt.ValueKind != JsonValueKind.Number || !resetsAt.TryGetInt64(out var unixSeconds))
            return null;
