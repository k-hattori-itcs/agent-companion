...
public class PetManager
{
    ...
    private static readonly JsonSerializerOptions ManifestWriteOptions = new() { WriteIndented = true, DefaultIgnoreCondition = JsonIgnoreCondition.WhenWritingNull };
    ...
    public PetManager()
        : this(AppDataPaths.PetsDirectory)
    {
    }
    ...
    public string? ImportPet(string zipPath)
    {
        ...
        try
        {
            ...
            var spriteDimensions = SpriteLoader.ValidateImage(stagedSpritePath);
            var spriteMetadataError = ValidateSpriteMetadata(info, spriteDimensions);
            if (spriteMetadataError != null)
                return spriteMetadataError;
            ...
            InstallPetFiles(stagingDir, petDir, previewEntry != null);
    ...
    // Replacing individual files keeps imports working while Explorer or the active
    // character renderer holds a directory handle on the existing character folder.
    private static void InstallPetFiles(string stagingDir, string petDir, bool includesPreview)
    {
        if (!Directory.Exists(petDir))
            Directory.CreateDirectory(petDir);

        var stagedFiles = Directory.GetFiles(stagingDir)
            .OrderBy(file => Path.GetFileName(file).Equals("pet.json", StringComparison.OrdinalIgnoreCase) ? 1 : 0);
        foreach (var stagedFile in stagedFiles)
        {
            var destination = Path.Combine(petDir, Path.GetFileName(stagedFile));
            var temporaryPath = destination + $".{Guid.NewGuid():N}.tmp";
            try
            {
                File.Copy(stagedFile, temporaryPath, overwrite: false);
        ...
        if (!includesPreview)
        {
            var stalePreview = Path.Combine(petDir, "preview-idle.png");
            if (File.Exists(stalePreview))
    ...
    private static void WriteNormalizedManifest(string directory, PetInfo info)
    {
        var manifest = new
        {
            ...
            spritesheetLayout = info.SpritesheetLayout,
            animationScales = info.AnimationScales.Count == 0 ? null : info.AnimationScales
        };
        var json = JsonSerializer.Serialize(manifest, ManifestWriteOptions);
        File.WriteAllText(Path.Combine(directory, "pet.json"), json);
    }

    private static string? ValidateSpriteMetadata(PetInfo info, (int Width, int Height) dimensions)
    {
        if (info.SpriteVersionNumber is null or 1)
            return null;
        if (info.SpriteVersionNumber != 2)
            return "pet.json のspriteVersionNumberが未対応です。";

        if (info.SpritesheetLayout is { } layout &&
            (layout.Columns != 8 || layout.Rows != 11 || layout.CellWidth != Models.AnimationDefs.FrameWidth ||
             layout.CellHeight != Models.AnimationDefs.FrameHeight || layout.LookDirectionCount != 16))
        {
            return "pet.json のspritesheetLayoutがv2仕様と一致しません。";
        }

        var expectedWidth = Models.AnimationDefs.FrameWidth * 8;
        var expectedHeight = Models.AnimationDefs.FrameHeight * 11;
        return dimensions.Width == expectedWidth && dimensions.Height == expectedHeight
            ? null
            : "v2キャラクターのspritesheetは1536x2288pxである必要があります。";
    }
    ...
    private static PetInfo? LoadPetInfo(string dir)
    {
        ...
        try
        {
            ...
            var manifest = JsonSerializer.Deserialize<PetInfo>(json, ManifestReadOptions);
            if (manifest == null)
                return null;

            using var doc = JsonDocument.Parse(json);
            var animationScales = ReadAnimationScales(doc.RootElement, manifest.Id);
            var info = manifest with
            {
    ...
    private static IReadOnlyDictionary<string, double> ReadAnimationScales(JsonElement root, string petId)
    {
        var scales = new Dictionary<string, double>(StringComparer.Ordinal);
        if (root.TryGetProperty("animationScales", out var configured) && configured.ValueKind == JsonValueKind.Object)
        {
            foreach (var property in configured.EnumerateObject())
            {
                if (Models.AnimationDefs.GetByName(property.Name) == null ||
                    !property.Value.TryGetDouble(out var scale) ||
                    !double.IsFinite(scale))
                {
                    continue;
                }

                scales[property.Name] = Math.Clamp(scale, 0.6, 1.0);
            }
        }

        if (petId.Equals("luna", StringComparison.OrdinalIgnoreCase) && !scales.ContainsKey("sit"))
            scales["sit"] = 0.9;
        if (petId.Equals("natsuki", StringComparison.OrdinalIgnoreCase))
        {
            if (!scales.ContainsKey("jump"))
                scales["jump"] = 0.9;
            if (!scales.ContainsKey("sit"))
                scales["sit"] = 0.84;
        }
