...
public partial class App : Application
{
    ...
    {
        var petsDir = AppDataPaths.PetsDirectory;
    ...
    internal static void ExtractEmbeddedPets(Assembly assembly, string petsDir)
    {
        ...
        var hashesPath = Path.Combine(root, ".bundled-assets.json");
        var previousHashes = ReadBundledAssetHashes(hashesPath);
        ...
        foreach (var name in assembly.GetManifestResourceNames())
        {
            ...
            var resourcePath = name[5..].Replace('\\', '/');
            ...
            var bundledHash = Convert.ToHexString(SHA256.HashData(data));
            bundledHashes[resourcePath] = bundledHash;

            var shouldWrite = !File.Exists(destination);
            if (!shouldWrite)
            {
                using var existing = new FileStream(destination, FileMode.Open, FileAccess.Read, FileShare.ReadWrite | FileShare.Delete);
                var existingHash = Convert.ToHexString(SHA256.HashData(existing));
                shouldWrite = previousHashes.TryGetValue(resourcePath, out var previousHash)
                    && existingHash.Equals(previousHash, StringComparison.OrdinalIgnoreCase)
                    && !existingHash.Equals(bundledHash, StringComparison.OrdinalIgnoreCase);
            }
            ...
            WriteBundledAsset(destination, data);
        }

        AtomicFile.WriteAllText(hashesPath, JsonSerializer.Serialize(bundledHashes));
    }

    private static Dictionary<string, string> ReadBundledAssetHashes(string path)
    {
        if (!File.Exists(path))
        ...
        {
            AppLogger.Warning($"Bundled character hash data could not be read: {ex.GetType().Name}");
            return new Dictionary<string, string>(StringComparer.Ordinal);
        }
    }

    private static void WriteBundledAsset(string destination, byte[] data)
    {
